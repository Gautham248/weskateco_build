...
export default function SafeStructuredSection() {
  const [activeId, setActiveId] = useState("coaches");
  ...
  return (
    <section className="w-full bg-[#F4F4F6] py-12 md:py-[120px]">
      <div className="mx-auto w-full max-w-(--breakpoint-2xl) px-4 lg:px-15 flex flex-col gap-8 md:gap-12">
        ...
        {/* Main Grid: Left Image + Right Interactive Feature List */}
            ...
            {SAFETY_FEATURES.map((item) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center text-left transition-colors cursor-pointer group w-fit"
                >
                  {/* Arrow Circle Badge */}
                  <div
                    className={`transition-all duration-300 ease-out flex items-center justify-center shrink-0 rounded-full bg-[#CCFF02] ${isActive
                  ...
                  {/* Feature Label */}
                  <span
                    className={`text-xl md:text-[32px] leading-[120%] tracking-[-2%] transition-colors ${isActive
