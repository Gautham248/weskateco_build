...
export default async function CategoryPage(props: {
  ...
  const params = await props.params;
  const { sort, color, level, price, page } = (searchParams || {}) as { [key: string]: string };
  ...
  if (!parentCategory && activeHandle) {
    ...
    } else if (handleLower.includes("protect") || handleLower.includes("helmet") || handleLower.includes("pad")) {
  ...
  if (parentCategory) {
    ...
    if (prefix) {
      filteredCollections = collections.filter(c => c.title.toLowerCase().startsWith(prefix));
    }
  ...
  const formattedCollections = filteredCollections
    .filter(c => {
      ...
      if (titleLower === "skateboard and surfskate full-setups" || handleLower === "surfboard full-setups") return false;
      return true;
    })
    .map(c => {
      let displayTitle = c.title;
      const isSameAsNav = displayTitle.toLowerCase() === activeHandle.toLowerCase() ||
  ...
  return (
    <section className="w-full">
      <StoreBanner title={collection.title} description={collection.description} />
            ...
            <div className="mt-12 flex items-center justify-center gap-2 py-4">
              ...
              <div className="flex items-center gap-1">
                {getPagesArray(currentPage, totalPages).map((p, idx) => {
                  if (p === '...') {
                    return (
                      <span key={`ellipse-${idx}`} className="px-2 text-neutral-400 dark:text-neutral-600">
                        ...
                      </span>
                    );
                  }
