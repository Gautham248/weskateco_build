...
async function dumpProducts() {
  ...
  const token = process.env.SHOPIFY_STOREFRONT_ACCESS_TOKEN || 'fbd019473f0c4aa6d82a713d782f6737';
  ...
  try {
    ...
    fs.writeFileSync('scripts/product-catalog-dump.json', JSON.stringify(products, null, 2));
    console.log(`Dumped ${products.length} products to scripts/product-catalog-dump.json`);
  } catch (error) {
